...
router.post('/dangerous-mode/enable', authenticate, requirePermission('dangerous', 'execute'), asyncHandler(async (req, res) => {
  const user = req.user!;
  ...
  const success = securityTracker.enableDangerousMode(user.sessionId, confirmation);
...
router.post('/dangerous-mode/disable', authenticate, asyncHandler(async (req, res) => {
  const user = req.user!;
  const securityTracker = req.app.get('securityTracker');
...
router.get('/export', authenticate, requireAdmin(), asyncHandler(async (req, res) => {
  ...
  const exportData = securityEventLogger.exportLogs(format as 'json' | 'csv', filter);
...
if (process.env.NODE_ENV === 'development') {
  router.post('/test-alert', authenticate, requireAdmin(), asyncHandler(async (req, res) => {
