...
import { requireAdmin, requirePermission, PermissionChecker } from '../auth/permissions.js';
...
router.get('/roles', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
...
router.get('/my-permissions', authenticate, async (req, res) => {
  try {
    ...
    const resources = ['project', 'session', 'adapter', 'cli', 'git', 'user', 'system', 'mcp', 'dangerous'];
    const availableActions = resources.reduce((acc, resource) => {
      acc[resource] = PermissionChecker.getAvailableActions(user.role, resource);
      return acc;
    ...
    res.json({
      ...
      data: {
        ...
        capabilities: {
          ...
          canPerformBulkOperations: PermissionChecker.canPerformBulkOperations(user.role)
        }
      }
    });
  } catch (error) {
...
router.put('/users/bulk-role', authenticate, requireAdmin(), async (req, res) => {
  try {
