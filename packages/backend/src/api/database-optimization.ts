      ...
      const metrics = {
        ...
        queries: {
          ...
          cache: {
            ...
            hitRatio:
              queryMetrics.cacheHits + queryMetrics.cacheMisses > 0
                ? Math.round(
                    (queryMetrics.cacheHits /
                      (queryMetrics.cacheHits + queryMetrics.cacheMisses)) *
                      100
                  )
                : 0,
          },
        ...
        analysis: {
          ...
          averageExecutionTime:
            Math.round(analysisReport.summary.averageExecutionTime * 100) / 100,
          recommendations: analysisReport.recommendations.length,
          performance: calculateOverallPerformance(
            queryMetrics,
            analysisReport
          ),
        },
  ...
  async (req, res) => {
    try {
      ...
      const analysis = {
        ...
        indexUsage: Array.from(report.indexUsage.entries()).map(
          ([index, count]) => ({
            index,
            usageCount: count,
          })
      ...
      const recommendations = {
        ...
        summary: {
          totalRecommendations:
            report.recommendations.length + indexSuggestions.length,
          critical: report.recommendations.filter(
  ...
  async (req, res) => {
    try {
      ...
      const slowQueries = [
        ...
        ...report.summary.topSlowQueries.filter(
          q => q.averageTime >= thresholdMs
        ),
      ]
        .sort((a, b) => {
    ...
    try {
      ...
      const schema = {
        ...
        summary: {
          ...
          totalRows: Object.values(dbInfo.tableSizes).reduce(
        ...
        metrics: {
          queryPerformance:
            report.summary.averageExecutionTime < 50
              ? 'excellent'
              : report.summary.averageExecutionTime < 100
                ? 'good'
                : report.summary.averageExecutionTime < 200
                  ? 'fair'
                  : 'poor',
          cacheEfficiency:
            queryMetrics.cacheHits + queryMetrics.cacheMisses > 0
...
router.get('/export', authenticate, requireAdmin(), async (req, res) => {
  try {
    ...
    if (format === 'csv') {
      ...
      res.setHeader(
        'Content-Disposition',
        'attachment; filename=database-performance.csv'
      );
      res.send(csvData);
    } else {
      ...
      res.setHeader(
        'Content-Disposition',
        'attachment; filename=database-performance.json'
      );
      res.json({
        success: true,
        data: exportData,
...
function calculateOverallPerformance(
  ...
  const slowQueryRatio =
    analysisReport.summary.totalQueries > 0
...
function convertToCSV(data: any): string {
  ...
  const rows = [
    ...
    [
      'Cache Hit Ratio',
      `${((data.queryMetrics.cacheHits / (data.queryMetrics.cacheHits + data.queryMetrics.cacheMisses)) * 100).toFixed(2)}%`,
    ],
  ];
