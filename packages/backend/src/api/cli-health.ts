 ...
 */
router.get('/status', authenticate, requirePermission('cli', 'read'), async (req, res) => {
  try {
    ...
    res.json({
      ...
      metadata: {
        ...
        healthyCLIs: healthStatuses.filter(s => s.status === 'healthy').length,
        unhealthyCLIs: healthStatuses.filter(s => s.status === 'unhealthy').length,
        degradedCLIs: healthStatuses.filter(s => s.status === 'degraded').length,
 ...
 */
router.get('/status/:cliName', authenticate, requirePermission('cli', 'read'), async (req, res) => {
  try {
 ...
 */
router.post('/check/:cliName', authenticate, requirePermission('cli', 'manage'), async (req, res) => {
  try {
    const { cliName } = req.params;
 ...
 */
router.post('/monitor/:cliName/start', authenticate, requirePermission('cli', 'manage'), async (req, res) => {
  try {
    const { cliName } = req.params;
    const userId = req.user?.id || 'unknown';
 ...
 */
router.post('/monitor/:cliName/stop', authenticate, requirePermission('cli', 'manage'), async (req, res) => {
  try {
    const { cliName } = req.params;
    const userId = req.user?.id || 'unknown';
 ...
 */
router.get('/diagnostics/:cliName', authenticate, requirePermission('cli', 'read'), async (req, res) => {
  try {
    const { cliName } = req.params;
 ...
 */
router.get('/config', authenticate, requirePermission('cli', 'read'), async (req, res) => {
  try {
 ...
 */
router.put('/config', authenticate, requirePermission('cli', 'manage'), async (req, res) => {
  try {
    const userId = req.user?.id || 'unknown';
 ...
 */
router.get('/metrics', authenticate, requirePermission('cli', 'read'), async (req, res) => {
  try {
    const healthStatuses = cliHealthMonitor.getAllHealthStatuses();
    ...
    const metrics = {
      overview: {
        ...
        degradedCLIs: healthStatuses.filter(s => s.status === 'degraded').length,
        unhealthyCLIs: healthStatuses.filter(s => s.status === 'unhealthy').length,
        unknownCLIs: healthStatuses.filter(s => s.status === 'unknown').length,
      },
      performance: {
        averageResponseTime: healthStatuses.reduce((sum, s) => sum + (s.responseTime || 0), 0) / healthStatuses.length,
        totalCommands: healthStatuses.reduce((sum, s) => sum + s.performance.commandCount, 0),
        totalErrors: healthStatuses.reduce((sum, s) => sum + s.performance.errorCount, 0),
        overallSuccessRate: healthStatuses.length > 0 
          ? healthStatuses.reduce((sum, s) => sum + s.performance.successRate, 0) / healthStatuses.length
          : 1.0,
      },
 ...
 */
router.get('/history/:cliName', authenticate, requirePermission('cli', 'read'), async (req, res) => {
  try {
    const { cliName } = req.params;
 ...
 */
router.get('/export', authenticate, requirePermission('cli', 'read'), async (req, res) => {
  try {
    ...
    if (format === 'json') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="cli-health-${Date.now()}.json"`);
      res.json(exportData);
    } else {
      res.status(400).json({
        success: false,
