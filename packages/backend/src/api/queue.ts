...
import { processQueueManager, JobType, JobPriority, JobStatus } from '../queue/index.js';
 ...
 */
router.get('/metrics/:jobType?', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    ...
    const metrics = await processQueueManager.getQueueMetrics(jobType as JobType);
    
    res.json({
      success: true,
      metrics: jobType ? metrics : Object.fromEntries(metrics as Map<JobType, any>),
    });
  } catch (error) {
 ...
 */
router.get('/:jobType/status', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    const { jobType } = req.params;
    ...
    const status = await processQueueManager.getQueueStatus(jobType as JobType);
 ...
 */
router.get('/:jobType/jobs/:status', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
 ...
 */
router.post('/:jobType/jobs', authenticate, requirePermission('system', 'write'), async (req, res) => {
  try {
    const { jobType } = req.params;
    ...
    if (options.priority && !Object.values(JobPriority).includes(options.priority)) {
      return res.status(400).json({
        success: false,
 ...
 * Get a specific job
 */
router.get('/:jobType/jobs/:jobId', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    ...
    const job = await processQueueManager.getJob(jobType as JobType, sanitizeInput(jobId));
 ...
 * Cancel a job
 */
router.delete('/:jobType/jobs/:jobId', authenticate, requirePermission('system', 'write'), async (req, res) => {
  try {
    const { jobType, jobId } = req.params;
    const userId = req.user?.id;
    ...
    const success = await processQueueManager.cancelJob(jobType as JobType, sanitizeInput(jobId));
 ...
 * Retry a failed job
 */
router.post('/:jobType/jobs/:jobId/retry', authenticate, requirePermission('system', 'write'), async (req, res) => {
  try {
    const { jobType, jobId } = req.params;
    const userId = req.user?.id;
    ...
    const success = await processQueueManager.retryJob(jobType as JobType, sanitizeInput(jobId));
 ...
 */
router.post('/:jobType/pause', authenticate, requireAdmin(), async (req, res) => {
  try {
    const { jobType } = req.params;
    const userId = req.user?.id;
 ...
 */
router.post('/:jobType/resume', authenticate, requireAdmin(), async (req, res) => {
  try {
    const { jobType } = req.params;
    const userId = req.user?.id;
 ...
 */
router.post('/:jobType/clean', authenticate, requireAdmin(), async (req, res) => {
  try {
    const { jobType } = req.params;
