...
import {
  getEnhancedMessageBatcher,
  createEnhancedMessageBatcher,
} from '../websocket/enhanced-message-batcher.js';
import {
  getPoolOptimizer,
  createPoolOptimizer,
} from '../websocket/pool-optimizer.js';
import {
      ...
      const combinedMetrics = {
        ...
        poolOptimizer: optimizerMetrics
          ? {
              ...optimizerMetrics,
              status:
                optimizerMetrics.totalOptimizations > 0 ? 'active' : 'idle',
            }
          : null,
        overall: {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const analytics = {
        connectionDistribution: analyzeConnectionDistribution(
          connectionPool,
          groupBy as string
        ),
  ...
  async (req, res) => {
    try {
      ...
      const monitoringData = {
        connectionPool: {
          status:
            poolStats.poolEfficiency > 0.9
              ? 'overloaded'
              : poolStats.poolEfficiency > 0.7
                ? 'busy'
                : 'healthy',
        ...
        quality: {
          ...
          averageQuality: calculateAverageQuality(
            poolStats.connectionsByQuality
          ),
        },
  ...
  async (req, res) => {
    try {
      ...
      const performanceRecommendations =
        generatePerformanceRecommendations(connectionPool);
...
function generateSystemRecommendations(stats: any): any[] {
  ...
  if (stats.poolEfficiency > 0.9) {
    recommendations.push({
      ...
      priority: 'high',
      message:
        'Connection pool efficiency is very high. Consider increasing capacity.',
...
function sanitizeBatcherConfig(config: any): any {
  return {
    maxBatchSize: Math.max(
      1,
      Math.min(100, parseInt(config.maxBatchSize) || 20)
    ),
    maxBatchDelay: Math.max(
      10,
      Math.min(1000, parseInt(config.maxBatchDelay) || 50)
    ),
    compressionThreshold: Math.max(
      256,
      Math.min(64 * 1024, parseInt(config.compressionThreshold) || 1024)
    ),
    ...
    messageTimeout: Math.max(
      1000,
      Math.min(300000, parseInt(config.messageTimeout) || 30000)
    ),
    adaptiveBatching: Boolean(config.adaptiveBatching),
    targetLatency: Math.max(
      10,
      Math.min(1000, parseInt(config.targetLatency) || 25)
    ),
    latencyAdjustmentFactor: Math.max(
      0.01,
      Math.min(1.0, parseFloat(config.latencyAdjustmentFactor) || 0.1)
    ),
  };
}
...
function sanitizeOptimizerConfig(config: any): any {
  return {
    ...
    targetPoolEfficiency: Math.max(
      0.1,
      Math.min(1.0, parseFloat(config.targetPoolEfficiency) || 0.75)
    ),
    maxLatencyThreshold: Math.max(
      50,
      Math.min(5000, parseInt(config.maxLatencyThreshold) || 200)
    ),
    minLatencyThreshold: Math.max(
      10,
      Math.min(500, parseInt(config.minLatencyThreshold) || 50)
    ),
    connectionUpgradeThreshold: Math.max(
      10,
      Math.min(200, parseInt(config.connectionUpgradeThreshold) || 50)
    ),
    connectionDowngradeThreshold: Math.max(
      100,
      Math.min(2000, parseInt(config.connectionDowngradeThreshold) || 500)
    ),
    scalingCooldownPeriod: Math.max(
      10000,
      Math.min(600000, parseInt(config.scalingCooldownPeriod) || 60000)
    ),
    optimizationInterval: Math.max(
      5000,
      Math.min(300000, parseInt(config.optimizationInterval) || 30000)
    ),
    enablePreemptiveCleanup: Boolean(config.enablePreemptiveCleanup),
    qualityCheckInterval: Math.max(
      5000,
      Math.min(60000, parseInt(config.qualityCheckInterval) || 15000)
    ),
  };
}
