...
import { adapterLifecycleManager, AdapterStatus } from '../adapters/lifecycle-manager.js';
 ...
 */
router.get('/status', authenticate, requirePermission('adapter', 'read'), async (req, res) => {
  try {
    const statuses = adapterLifecycleManager.getAllAdapterStatuses();
    const statusArray = Array.from(statuses.entries()).map(([adapterId, status]) => ({
      adapterId,
      status,
    ...
    res.json({
      ...
      statusCounts: Object.values(AdapterStatus).reduce((acc, status) => {
 ...
 */
router.get('/status/:status', authenticate, requirePermission('adapter', 'read'), async (req, res) => {
  try {
    ...
    const adapterIds = adapterLifecycleManager.getAdaptersByStatus(sanitizedStatus);
 ...
 */
router.get('/:adapterId', authenticate, requirePermission('adapter', 'read'), async (req, res) => {
  try {
    const { adapterId } = req.params;
    const sanitizedAdapterId = sanitizeInput(adapterId);

    const health = adapterLifecycleManager.getAdapterHealth(sanitizedAdapterId);
    const metrics = adapterLifecycleManager.getAdapterMetrics(sanitizedAdapterId);
    const history = adapterLifecycleManager.getAdapterLifecycleHistory(sanitizedAdapterId);
 ...
 * Start an adapter
 */
router.post('/:adapterId/start', authenticate, requirePermission('adapter', 'write'), async (req, res) => {
  try {
    const { adapterId } = req.params;
    const userId = req.user?.id;
    const sanitizedAdapterId = sanitizeInput(adapterId);

    const success = await adapterLifecycleManager.startAdapter(sanitizedAdapterId);

    if (!success) {
      return res.status(400).json({
        success: false,
        message: 'Failed to start adapter - adapter may not be in a startable state',
      });
    }
 ...
 * Stop an adapter
 */
router.post('/:adapterId/stop', authenticate, requirePermission('adapter', 'write'), async (req, res) => {
  try {
    const { adapterId } = req.params;
    ...
    const success = await adapterLifecycleManager.stopAdapter(sanitizedAdapterId, graceful);
 ...
 * Restart an adapter
 */
router.post('/:adapterId/restart', authenticate, requirePermission('adapter', 'write'), async (req, res) => {
  try {
    const { adapterId } = req.params;
    const userId = req.user?.id;
    const sanitizedAdapterId = sanitizeInput(adapterId);

    const success = await adapterLifecycleManager.restartAdapter(sanitizedAdapterId);
 ...
 */
router.get('/:adapterId/health', authenticate, requirePermission('adapter', 'read'), async (req, res) => {
  try {
    const { adapterId } = req.params;
    const sanitizedAdapterId = sanitizeInput(adapterId);
 ...
 * Get adapter metrics
 */
router.get('/:adapterId/metrics', authenticate, requirePermission('adapter', 'read'), async (req, res) => {
  try {
    const { adapterId } = req.params;
    const sanitizedAdapterId = sanitizeInput(adapterId);
 ...
 */
router.get('/:adapterId/history', authenticate, requirePermission('adapter', 'read'), async (req, res) => {
  try {
    const { adapterId } = req.params;
    ...
    const history = adapterLifecycleManager.getAdapterLifecycleHistory(sanitizedAdapterId, limitNum);
 ...
 */
router.post('/emergency-shutdown', authenticate, requireAdmin(), async (req, res) => {
  try {
 ...
 */
router.get('/metadata/states', authenticate, requirePermission('adapter', 'read'), (req, res) => {
  res.json({
    success: true,
    metadata: {
      ...
      statusDescriptions: {
        ...
        [AdapterStatus.DEGRADED]: 'Adapter is operational but with reduced performance',
