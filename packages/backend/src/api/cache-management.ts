...
function generateCacheRecommendations(stats: any, health: any): string[] {
  ...
  if (health.redis.memory > 1000000000) {
    // 1GB
  ...
  if (stats.errors > stats.hits * 0.01) {
    recommendations.push(
      'Error rate is high, review cache configuration and Redis connectivity'
    );
  }

  if (health.performance.hitRatio > 0.9) {
    recommendations.push(
      'Excellent cache performance, consider extending current strategies'
    );
  }
