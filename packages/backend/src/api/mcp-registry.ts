...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const serverId = await mcpServerRegistry.registerServer(
        serverDefinition,
        userId
      );
    ...
    try {
      ...
      res.status(500).json({
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to update server instance',
      });
    }
  }
);
    ...
    try {
      ...
      res.status(500).json({
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to delete server instance',
      });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const template =
        mcpServerRegistry.generateConfigurationTemplate(serverId);
    ...
    try {
      const categories = [
        ...
        },
        {
          id: 'custom',
          name: 'Custom',
          description: 'Custom or specialized servers',
        },
      ];
  ...
  async (req, res) => {
    try {
      ...
      const allowedFields = Object.keys(
        serverDefinition.configuration.schema.properties || {}
      );
  ...
  async (req, res) => {
    try {
      ...
      if (includeInstances === 'true') {
        exportData.data.instances =
          mcpServerRegistry.getServerInstances(userId);
      }
      ...
      if (format === 'json') {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="mcp-registry-${Date.now()}.json"`
        );
        res.json(exportData);
      } else {
        res.status(400).json({
