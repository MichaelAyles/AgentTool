...
router.get(
  '/status/:projectPath(*)/visualization',
  authenticate,
  requirePermission('git', 'read'),
  async (req, res) => {
    try {
      ...
      res.status(500).json({
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to get git status visualization',
      });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to get git tree',
      });
    }
  }
);
...
router.post('/pull/:projectPath(*)', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to pull from remote',
    });
  }
});
...
router.post('/worktree/:projectPath(*)/unlock', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        error instanceof Error ? error.message : 'Failed to unlock worktree',
    });
  }
});
...
router.post(
  '/operations/stage/:projectPath(*)',
  authenticate,
  requirePermission('git', 'write'),
  async (req, res) => {
    try {
      const projectPath = decodeURIComponent(req.params.projectPath);
      ...
      const result = await operations.stageFiles({
        files: files || [],
        all,
        update,
      });
...
router.post(
  '/operations/unstage/:projectPath(*)',
  authenticate,
  requirePermission('git', 'write'),
  async (req, res) => {
    try {
...
router.post(
  '/operations/push/:projectPath(*)',
  authenticate,
  requirePermission('git', 'dangerous'),
  async (req, res) => {
    try {
      const projectPath = decodeURIComponent(req.params.projectPath);
      const {
...
router.post(
  '/operations/branch/create/:projectPath(*)',
  authenticate,
  requirePermission('git', 'write'),
  async (req, res) => {
    try {
...
router.post(
  '/operations/branch/checkout/:projectPath(*)',
  authenticate,
  requirePermission('git', 'write'),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to reset repository',
      });
    }
  }
);
...
router.post(
  '/operations/stash/pop/:projectPath(*)',
  authenticate,
  requirePermission('git', 'write'),
  async (req, res) => {
    try {
