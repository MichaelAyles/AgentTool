 ...
 */
router.get('/types', authenticate, requirePermission('script', 'read'), async (req, res) => {
  try {
    // This would normally get the types from the adapter registry
    const scriptTypes = [
      { type: 'bash', interpreter: 'bash', extension: '.sh', description: 'Bash shell scripts' },
      { type: 'zsh', interpreter: 'zsh', extension: '.zsh', description: 'Z shell scripts' },
      { type: 'sh', interpreter: 'sh', extension: '.sh', description: 'POSIX shell scripts' },
      { type: 'python', interpreter: 'python3', extension: '.py', description: 'Python 3 scripts' },
      { type: 'node', interpreter: 'node', extension: '.js', description: 'Node.js JavaScript' },
      { type: 'deno', interpreter: 'deno', extension: '.ts', description: 'Deno TypeScript/JavaScript' },
      { type: 'bun', interpreter: 'bun', extension: '.ts', description: 'Bun TypeScript/JavaScript' },
      { type: 'ruby', interpreter: 'ruby', extension: '.rb', description: 'Ruby scripts' },
      { type: 'php', interpreter: 'php', extension: '.php', description: 'PHP scripts' },
      { type: 'go', interpreter: 'go', extension: '.go', description: 'Go programs' },
      { type: 'rust', interpreter: 'rustc', extension: '.rs', description: 'Rust programs' },
      { type: 'jq', interpreter: 'jq', extension: '.jq', description: 'JSON query processor' },
      { type: 'awk', interpreter: 'awk', extension: '.awk', description: 'AWK text processing' },
      { type: 'sed', interpreter: 'sed', extension: '.sed', description: 'Stream editor' },
      { type: 'custom', interpreter: '', extension: '.txt', description: 'Custom interpreter' },
    ];
 ...
 */
router.post('/execute', authenticate, requirePermission('script', 'execute'), async (req, res) => {
  try {
    const {
      script,
      scriptType = 'bash',
 ...
 */
router.get('/saved', authenticate, requirePermission('script', 'read'), async (req, res) => {
  try {
    const userId = req.user?.id || 'unknown';
    ...
    if (isPublic !== undefined) {
      ...
      filteredScripts = filteredScripts.filter(script =>
        script.isPublic === publicFilter
      );
    }

    // Sort by modified date (newest first)
    filteredScripts.sort((a, b) => b.modified.getTime() - a.modified.getTime());
 ...
 */
router.get('/saved/:scriptId', authenticate, requirePermission('script', 'read'), async (req, res) => {
  try {
 ...
 * Save a new script
 */
router.post('/saved', authenticate, requirePermission('script', 'create'), async (req, res) => {
  try {
    const {
      name,
      description = '',
 ...
 * Update a script
 */
router.put('/saved/:scriptId', authenticate, requirePermission('script', 'write'), async (req, res) => {
  try {
    const { scriptId } = req.params;
    const {
      name,
    ...
    const updatedScript: StoredScript = {
      ...
      description: description !== undefined ? sanitizeInput(description) : script.description,
      scriptType: scriptType !== undefined ? sanitizeInput(scriptType) : script.scriptType,
      content: content !== undefined ? content : script.content,
      tags: tags !== undefined ? (Array.isArray(tags) ? tags.map(tag => sanitizeInput(tag)) : script.tags) : script.tags,
 ...
 * Delete a script
 */
router.delete('/saved/:scriptId', authenticate, requirePermission('script', 'delete'), async (req, res) => {
  try {
    const { scriptId } = req.params;
    const userId = req.user?.id || 'unknown';
    const sanitizedScriptId = sanitizeInput(scriptId);
 ...
 */
router.post('/saved/:scriptId/execute', authenticate, requirePermission('script', 'execute'), async (req, res) => {
  try {
    const { scriptId } = req.params;
    const {
      workingDirectory,
 ...
 */
router.get('/public', authenticate, requirePermission('script', 'read'), async (req, res) => {
  try {
    const { tags, scriptType, author } = req.query;

    let publicScripts = Array.from(scripts.values())
