...
router.post(
  '/schemas/:adapterId',
  authenticate,
  requirePermission('adapter', 'configure'),
  async (req, res) => {
    try {
      const adapterId = sanitizeInput(req.params.adapterId);
      ...
      res.status(500).json({
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to register adapter schema',
      });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      const configurations =
        adapterConfigManager.getAdapterConfigurations(adapterId);
 ...
 */
router.get(
  '/configurations/:adapterId/active',
  authenticate,
  requirePermission('adapter', 'read'),
  async (req, res) => {
    try {
      const adapterId = sanitizeInput(req.params.adapterId);
      const activeConfig =
        adapterConfigManager.getActiveConfiguration(adapterId);
...
router.put(
  ...
  async (req, res) => {
    try {
      const configId = sanitizeInput(req.params.configId);
      const { name, description, configuration, tags, metadata, changeReason } =
        req.body;
      const userId = req.user?.id || 'unknown';
 ...
 */
router.post(
  '/configurations/:adapterId/active/:configId',
  authenticate,
  requirePermission('adapter', 'configure'),
  async (req, res) => {
    try {
      ...
      await adapterConfigManager.setActiveConfiguration(
        adapterId,
        configId,
        userId
      );
 ...
 */
router.post(
  '/configuration/:configId/validate',
  authenticate,
  requirePermission('adapter', 'read'),
  async (req, res) => {
    try {
 ...
 */
router.get(
  '/configuration/:configId/history',
  authenticate,
  requirePermission('adapter', 'read'),
  async (req, res) => {
    try {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="${configuration.name}-config.json"`
      );
      res.json(exportData);
    } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const configName = sanitizeInput(
        name || `Imported ${importData.metadata.configurationName}`
      );
