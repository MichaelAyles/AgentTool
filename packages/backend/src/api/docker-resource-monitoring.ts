...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const trends = resourceMonitor.getResourceTrends(
        containerId,
 ...
 */
router.post(
  '/containers/:containerId/limits',
  authenticate,
  requirePermission('container', 'update'),
  async (req, res) => {
    try {
      const { containerId } = req.params;
      ...
      const success = await resourceMonitor.setResourceLimits(
        containerId,
        limits
      );
 ...
 */
router.get(
  '/containers/:containerId/predictions',
  authenticate,
  requirePermission('container', 'read'),
  async (req, res) => {
    try {
...
router.get(
  ...
  async (req, res) => {
    try {
      const summary = resourceMonitor.getUtilizationSummary();
      const criticalAlerts = resourceMonitor
        .getAllAlerts()
      ...
      res.json({
        ...
        data: {
          ...
          systemMetrics: {
            ...
            totalMemoryGB:
              Math.round((summary.totalMemoryUsed / 1024 / 1024 / 1024) * 100) /
              100,
          },
          timestamp: new Date().toISOString(),
        },
  ...
  async (req, res) => {
    try {
      ...
      if (format === 'csv') {
        ...
        const csvHeader =
          'timestamp,cpu_usage,memory_usage,memory_percentage,network_rx,network_tx,disk_read,disk_write,processes\n';
        const csvData = filteredMetrics
          .map(
            m =>
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="container-${containerId}-metrics.csv"`
        );
