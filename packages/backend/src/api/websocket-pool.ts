...
router.get(
  '/stats',
  authenticate,
  requirePermission('websocket', 'read'),
  async (req, res) => {
    try {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (
        sanitizedUserId !== requestingUserId &&
      ...
      const userConnections =
        connectionPool.getUserConnections(sanitizedUserId);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const success = await connectionPool.removeConnection(
        sanitizedSocketId,
        sanitizedReason
      );
