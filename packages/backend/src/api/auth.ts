...
const loginSchema = Joi.object({
  provider: Joi.string().valid('local', 'github', 'google', 'microsoft').required(),
...
// Login endpoint
router.post('/login', sanitizeInput(), asyncHandler(async (req, res) => {
...
// Revoke all sessions
router.post('/revoke-sessions', authenticate, asyncHandler(async (req, res) => {
  const userId = req.user!.id;
...
// OAuth URLs
router.get('/oauth/github/url', asyncHandler(async (req, res) => {
  const state = req.query.state as string;
  const url = authService.getOAuthUrl('github', state);
  res.json({ url });
}));

router.get('/oauth/google/url', asyncHandler(async (req, res) => {
  const state = req.query.state as string;
  const url = authService.getOAuthUrl('google', state);
  res.json({ url });
}));

router.get('/oauth/microsoft/url', asyncHandler(async (req, res) => {
  const state = req.query.state as string;
...
router.get('/oauth/github/callback', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (result.success && result.session) {
      ...
      res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}?auth=error&message=${encodeURIComponent(result.error || 'Authentication failed')}`);
    }
  } catch (error) {
    structuredLogger.error('OAuth callback error', error);
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}?auth=error&message=${encodeURIComponent('Authentication failed')}`);
  }
}));
...
router.get('/oauth/microsoft/callback', asyncHandler(async (req, res) => {
  const { code, state } = req.query;
