...
import {
  ...
  profileManager,
} from '../docker/security-profiles.js';
...
router.post(
  '/sandboxes',
  authenticate,
  requirePermission('sandbox', 'create'),
  async (req, res) => {
    try {
      const {
      ...
      res.status(201).json({
        ...
        data: {
          ...
          recommendations:
            SecurityPolicyEnforcer.getSecurityRecommendations(profile),
        },
      });
    } catch (error) {
...
router.post(
  ...
  authenticate,
  requirePermission('sandbox', 'execute'),
  async (req, res) => {
    try {
      ...
      const profile = profileManager.getProfile(
        sandbox.config.environmentVariables?.SECURITY_PROFILE || 'SAFE_MODE'
      );
      if (!profile) {
      ...
      const validation = SecurityPolicyEnforcer.validateCommand(
        fullCommand,
        profile
      );
      ...
      if (environment) {
        const envValidation = SecurityPolicyEnforcer.validateEnvironment(
          environment,
          profile
        );
  ...
  async (req, res) => {
    try {
      ...
      const summary = {
        ...
        totalMemoryUsage: sandboxes.reduce(
          (sum, s) => sum + s.resourceUsage.memory,
          0
        ),
        totalCpuUsage: sandboxes.reduce(
          (sum, s) => sum + s.resourceUsage.cpu,
          0
        ),
      };
...
router.delete(
  '/sandboxes/:sandboxId',
  authenticate,
  requirePermission('sandbox', 'delete'),
  async (req, res) => {
    try {
      const { sandboxId } = req.params;
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const profilesWithRecommendations = filteredProfiles.map(
        ({ name, profile }) => ({
          name,
          profile: {
            ...profile,
      ...
      res.json({
        ...
        data: {
          ...
          recommended: {
            untrusted_code:
              profileManager.getRecommendedProfile('untrusted_code'),
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const commandValidation = SecurityPolicyEnforcer.validateCommand(
        command,
        profile
      );
      const envValidation = SecurityPolicyEnforcer.validateEnvironment(
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const success = await containerRegistry.removeImage(
        imageId,
        Boolean(force)
      );
