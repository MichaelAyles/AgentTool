...
import { mcpBridge, MCPServerConfig, MCPRequest } from '../services/mcp-bridge.js';
...
router.get('/servers', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    ...
    const sanitizedServers = servers.map(server => ({
      ...
      config: {
        ...server.config,
        env: server.config.env ? Object.keys(server.config.env).reduce((acc, key) => {
          acc[key] = server.config.env![key].includes('KEY') || server.config.env![key].includes('TOKEN') 
            ? '[REDACTED]' 
 ...
 */
router.get('/servers/running', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
 ...
 */
router.post('/servers', authenticate, requirePermission('mcp', 'configure'), async (req, res) => {
  try {
    const {
      id,
      name,
 ...
 * Start an MCP server
 */
router.post('/servers/:serverId/start', authenticate, requirePermission('mcp', 'execute'), async (req, res) => {
  try {
    const serverId = sanitizeInput(req.params.serverId);
    const userId = req.user?.id || 'unknown';
 ...
 * Stop an MCP server
 */
router.post('/servers/:serverId/stop', authenticate, requirePermission('mcp', 'execute'), async (req, res) => {
  try {
    const serverId = sanitizeInput(req.params.serverId);
    const userId = req.user?.id || 'unknown';
 ...
 */
router.delete('/servers/:serverId', authenticate, requirePermission('mcp', 'configure'), async (req, res) => {
  try {
    const serverId = sanitizeInput(req.params.serverId);
    const userId = req.user?.id || 'unknown';
 ...
 * Execute MCP request
 */
router.post('/servers/:serverId/execute', authenticate, requirePermission('mcp', 'execute'), async (req, res) => {
  try {
    const serverId = sanitizeInput(req.params.serverId);
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to execute MCP request',
    });
  }
});
 ...
 */
router.get('/servers/:serverId/capabilities', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const serverId = sanitizeInput(req.params.serverId);
 ...
 */
router.get('/tools', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const runningServers = mcpBridge.getRunningServers();
    const allTools: any[] = [];
 ...
 */
router.get('/resources', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const runningServers = mcpBridge.getRunningServers();
 ...
 */
router.get('/prompts', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const runningServers = mcpBridge.getRunningServers();
 ...
 */
router.get('/status', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    ...
    const status = {
      ...
      errorServers: allServers.filter(s => s.state === 'error' || s.state === 'crashed').length,
