...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await mcpMessageHandler.sendRequest(
        connectionId,
        method,
        params,
        userId
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await mcpMessageHandler.sendNotification(
        connectionId,
        method,
        params,
        userId
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : 'Failed to test handler',
      });
    }
  }
);
...
function getMethodDescription(method: MCPMethod): string {
  const descriptions: Record<MCPMethod, string> = {
    ...
    [MCPMethod.NOTIFICATIONS_INITIALIZED]:
      'Initialization completed notification',
    [MCPMethod.NOTIFICATIONS_TOOLS_CHANGED]: 'Tools list changed notification',
    [MCPMethod.NOTIFICATIONS_RESOURCES_CHANGED]:
      'Resources list changed notification',
    [MCPMethod.NOTIFICATIONS_PROMPTS_CHANGED]:
