...
import {
  getOutputStreamOptimizer,
  defaultStreamingConfig,
} from '../streaming/output-optimizer.js';
import {
  getMessageBatcherFactory,
  defaultBatchConfig,
} from '../streaming/message-batcher.js';
import {
    ...
    try {
      ...
      const combinedMetrics = {
        streaming: {
          ...streamingMetrics,
          efficiency:
            streamingMetrics.compressionRatio *
            (1 - streamingMetrics.bufferUtilization),
          status:
            streamingMetrics.bufferUtilization > 0.9
              ? 'overloaded'
              : streamingMetrics.bufferUtilization > 0.7
                ? 'busy'
                : 'healthy',
        },
        batching: {
          ...
          status:
            batchingMetrics.queueLength > 100
              ? 'overloaded'
              : batchingMetrics.queueLength > 50
                ? 'busy'
                : 'healthy',
        },
        overall: {
          ...
          overallLatency:
            (streamingMetrics.averageLatency + batchingMetrics.averageLatency) /
            2,
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const streamOptimizer = getOutputStreamOptimizer();
      const bufferStatus =
        streamOptimizer.getSessionBufferStatus(sanitizedSessionId);
    ...
    try {
      ...
      const status = {
        streaming: {
          healthy:
            streamingMetrics.bufferUtilization < 0.9 &&
            streamingMetrics.averageLatency < 100,
        ...
        batching: {
          healthy:
            batchingMetrics.queueLength < 100 &&
            batchingMetrics.averageLatency < 50,
...
function generatePerformanceRecommendations(
  ...
  if (streamingMetrics.bufferUtilization > 0.9) {
    recommendations.push({
      ...
      priority: 'high',
      message:
        'Buffer utilization is very high. Consider reducing buffer size or increasing flush frequency.',
  ...
  if (streamingMetrics.averageLatency > 100) {
    recommendations.push({
      ...
      priority: 'medium',
      message:
        'Average streaming latency is high. Consider optimizing compression or reducing chunk size.',
  ...
  if (streamingMetrics.compressionRatio > 0.9) {
    recommendations.push({
      ...
      priority: 'low',
      message:
        'Compression ratio is low. Consider adjusting compression threshold or algorithm.',
  ...
  if (batchingMetrics.queueLength > 100) {
    recommendations.push({
      ...
      priority: 'high',
      message:
        'Message queue length is very high. Consider reducing batch delay or increasing batch size.',
  ...
  if (batchingMetrics.averageBatchSize < 3) {
    recommendations.push({
      ...
      priority: 'medium',
      message:
        'Average batch size is small. Consider increasing batch delay to improve efficiency.',
  ...
  if (
    streamingConfig.bufferSize > 128 * 1024 &&
    streamingMetrics.bufferUtilization < 0.3
  ) {
    recommendations.push({
      type: 'configuration',
      priority: 'low',
      message:
        'Buffer size might be too large for current usage. Consider reducing to save memory.',
      action: 'reduce_buffer_size',
    });
  }
...
function sanitizeStreamingConfig(config: any): any {
  return {
    bufferSize: Math.max(
      1024,
      Math.min(1024 * 1024, parseInt(config.bufferSize) || 64 * 1024)
    ),
    flushInterval: Math.max(
      10,
      Math.min(5000, parseInt(config.flushInterval) || 100)
    ),
    ...
    maxChunkSize: Math.max(
      1024,
      Math.min(256 * 1024, parseInt(config.maxChunkSize) || 32 * 1024)
    ),
