 ...
 */
router.get('/dashboard', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    ...
    const applicationMetrics = await performanceMonitor.getApplicationMetrics();
    
    // Get key performance indicators
    const responseTimeMetrics = performanceMonitor.getMetricsSummary('http.request.duration', timeRange);
    const requestRateMetrics = performanceMonitor.getMetricsSummary('http.request.count', timeRange);
    const errorMetrics = performanceMonitor.getMetricsSummary('error.occurrence', timeRange);
    ...
    const dashboard = {
      ...
      application: {
        requests: {
          ...
          perSecond: Math.round(applicationMetrics.requests.perSecond * 100) / 100,
          averageResponseTime: Math.round(applicationMetrics.requests.averageResponseTime),
          errorRate: Math.round(applicationMetrics.requests.errorRate * 10000) / 100, // percentage
        },
      ...
      health: {
        status: calculateOverallHealth(systemMetrics, applicationMetrics, criticalAlerts),
        scores: {
          performance: calculatePerformanceScore(responseTimeMetrics.summary),
          reliability: calculateReliabilityScore(applicationMetrics.requests.errorRate),
          efficiency: calculateEfficiencyScore(systemMetrics, applicationMetrics),
        },
      },
    };
 ...
 */
router.get('/metrics/:metricName', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    const { metricName } = req.params;
    const timeRange = getTimeRangeFromQuery(req.query);
    const { aggregation = 'none', interval = '5m' } = req.query;
    
    const metrics = performanceMonitor.getMetricsSummary(metricName, timeRange);
    
    let processedData = metrics.metrics;
    
    // Apply aggregation if requested
    if (aggregation !== 'none') {
      processedData = aggregateMetrics(metrics.metrics, interval as string, aggregation as string);
    }
 ...
 * Get system metrics
 */
router.get('/system', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    const systemMetrics = await performanceMonitor.getSystemMetrics();
    
    // Add historical data
    const cpuHistory = performanceMonitor.getMetricsSummary('cpu.usage', getTimeRangeFromQuery({ hours: '1' }));
    const memoryHistory = performanceMonitor.getMetricsSummary('memory.percentage', getTimeRangeFromQuery({ hours: '1' }));
 ...
 */
router.get('/application', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    const applicationMetrics = await performanceMonitor.getApplicationMetrics();
    const timeRange = getTimeRangeFromQuery(req.query);
    
    // Get detailed breakdowns
    const endpointMetrics = performanceMonitor.getMetricsSummary('api.usage.by_endpoint', timeRange);
    const userMetrics = performanceMonitor.getMetricsSummary('api.usage.by_user', timeRange);
    const responseTimeBreakdown = performanceMonitor.getMetricsSummary('http.request.duration', timeRange);
    
    const response = {
      overview: applicationMetrics,
      breakdowns: {
        topEndpoints: aggregateByTag(endpointMetrics.metrics, 'endpoint').slice(0, 10),
        topUsers: aggregateByTag(userMetrics.metrics, 'user_id').slice(0, 10),
        responseTimeDistribution: createDistribution(responseTimeBreakdown.metrics),
      },
      trends: {
        requests: calculateTrend(performanceMonitor.getMetricsSummary('http.request.count', timeRange).metrics),
        responseTime: calculateTrend(responseTimeBreakdown.metrics),
        errors: calculateTrend(performanceMonitor.getMetricsSummary('error.occurrence', timeRange).metrics),
      },
    };
 ...
 */
router.post('/alerts/:alertId/acknowledge', authenticate, requirePermission('system', 'write'), async (req, res) => {
  try {
  ...
  try {
    ...
    switch (format) {
      ...
      case 'prometheus':
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=metrics.prom');
        break;
      default:
 ...
 */
router.get('/insights', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    const timeRange = getTimeRangeFromQuery(req.query);
 ...
 */
router.get('/realtime', authenticate, requirePermission('system', 'read'), async (req, res) => {
  try {
    const systemMetrics = await performanceMonitor.getSystemMetrics();
    const applicationMetrics = await performanceMonitor.getApplicationMetrics();
    ...
    const recentRequests = performanceMonitor.getMetricsSummary('http.request.count', recentTimeRange);
    const recentResponseTime = performanceMonitor.getMetricsSummary('http.request.duration', recentTimeRange);
    const recentErrors = performanceMonitor.getMetricsSummary('error.occurrence', recentTimeRange);
    ...
    const realtime = {
      ...
      application: {
        ...
        errorRate: recentErrors.summary.count / Math.max(recentRequests.summary.count, 1),
        activeProcesses: applicationMetrics.processes.active,
        activeConnections: applicationMetrics.websockets.activeConnections,
      },
      alerts: {
        critical: performanceMonitor.getAlerts('critical').filter(a => !a.acknowledged).length,
        warnings: performanceMonitor.getAlerts('warning').filter(a => !a.acknowledged).length,
      },
    };
...
function getTimeRangeFromQuery(query: any): { start: Date; end: Date } {
  ...
  if (query.hours) {
    ...
    start = new Date(now.getTime() - parseInt(query.days) * 24 * 60 * 60 * 1000);
...
function calculateTrend(metrics: any[]): 'increasing' | 'decreasing' | 'stable' {
...
function calculateOverallHealth(system: any, application: any, criticalAlerts: any[]): string {
  if (criticalAlerts.length > 0) return 'critical';
  if (system.cpu.usage > 90 || system.memory.percentage > 95) return 'poor';
  if (application.requests.errorRate > 0.05 || application.requests.averageResponseTime > 2000) return 'degraded';
...
function aggregateMetrics(metrics: any[], interval: string, aggregation: string): any[] {
...
async function generatePerformanceInsights(timeRange: any): Promise<any> {
  ...
  const insights = {
    ...
    topIssues: [
      { issue: 'High response times on /api/projects endpoint', severity: 'medium' },
