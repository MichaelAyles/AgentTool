  ...
  async (req, res) => {
    try {
      ...
      await comprehensiveAuditLogger.logAuditEvent({
        ...
        outcome:
          successfulOperations === totalOperations ? 'success' : 'partial',
      ...
      res.json({
        ...
        summary: {
          ...
          resourcesReleased: results.reduce(
            (sum, r) => ({
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const history =
        processCleanupHandler.getCleanupHistory(sanitizedSessionId);
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ...
        recovered,
        message: recovered
          ? 'Error handled and process recovered'
          : 'Error handled but recovery failed',
      });
    } catch (error) {
