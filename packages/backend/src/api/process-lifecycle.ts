...
import { processStateMachine, ProcessState, ProcessEvent } from '../processes/state-machine.js';
 ...
 */
router.get('/active', authenticate, requirePermission('session', 'read'), async (req, res) => {
  try {
 ...
 */
router.post('/create', authenticate, requirePermission('session', 'create'), async (req, res) => {
  try {
    const {
      sessionId,
      adapterName,
    ...
    const existingState = processStateMachine.getState(sanitizeInput(sessionId));
    if (existingState) {
    ...
    const success = await processLifecycleManager.createProcess({
      ...
      workingDirectory: workingDirectory ? sanitizeInput(workingDirectory) : undefined,
    ...
    await comprehensiveAuditLogger.logAuditEvent({
      ...
      outcome: 'success',
      severity: dangerousModeEnabled ? SecurityLevel.DANGEROUS : SecurityLevel.SAFE,
      details: {
 ...
 * Get process status
 */
router.get('/:sessionId/status', authenticate, requirePermission('session', 'read'), async (req, res) => {
  try {
    ...
    const health = processLifecycleManager.getProcessHealth(sanitizedSessionId);
    const metrics = processLifecycleManager.getProcessMetrics(sanitizedSessionId);
    const history = processStateMachine.getLifecycleHistory(sanitizedSessionId);
 ...
 * Pause a process
 */
router.post('/:sessionId/pause', authenticate, requirePermission('session', 'write'), async (req, res) => {
  try {
    const { sessionId } = req.params;
    const userId = req.user?.id;
    const sanitizedSessionId = sanitizeInput(sessionId);

    const success = await processLifecycleManager.pauseProcess(sanitizedSessionId);

    if (!success) {
      return res.status(400).json({
        success: false,
        message: 'Failed to pause process - process may not be in a pausable state',
      });
    }
 ...
 * Resume a process
 */
router.post('/:sessionId/resume', authenticate, requirePermission('session', 'write'), async (req, res) => {
  try {
    const { sessionId } = req.params;
    const userId = req.user?.id;
    const sanitizedSessionId = sanitizeInput(sessionId);

    const success = await processLifecycleManager.resumeProcess(sanitizedSessionId);
 ...
 * Stop a process
 */
router.post('/:sessionId/stop', authenticate, requirePermission('session', 'write'), async (req, res) => {
  try {
    const { sessionId } = req.params;
    ...
    const success = await processLifecycleManager.stopProcess(sanitizedSessionId, graceful);
 ...
 * Restart a process
 */
router.post('/:sessionId/restart', authenticate, requirePermission('session', 'write'), async (req, res) => {
  try {
    const { sessionId } = req.params;
    const userId = req.user?.id;
    const sanitizedSessionId = sanitizeInput(sessionId);

    const success = await processLifecycleManager.restartProcess(sanitizedSessionId);
 ...
 */
router.post('/:sessionId/event', authenticate, requireAdmin(), async (req, res) => {
  try {
    const { sessionId } = req.params;
    ...
    const success = await processStateMachine.triggerEvent(sanitizedSessionId, event, metadata);
...
router.post('/cleanup', authenticate, requireAdmin(), async (req, res) => {
  try {
    ...
    const cleanedCount = await processLifecycleManager.cleanupTerminatedProcesses();
