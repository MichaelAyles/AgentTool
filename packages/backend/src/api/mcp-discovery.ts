 ...
 */
router.get('/tools/search', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
 ...
 */
router.get('/resources/search', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const userId = req.user?.id || 'unknown';
 ...
 */
router.get('/tools/:toolId', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
 ...
 */
router.get('/resources/:resourceId', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
 ...
 * Execute a tool
 */
router.post('/tools/:toolId/execute', authenticate, requirePermission('mcp', 'execute'), async (req, res) => {
  try {
    const { toolId } = req.params;
    const { parameters = {} } = req.body;
    const userId = req.user?.id || 'unknown';

    const result = await mcpDiscoveryService.executeTool(toolId, parameters, userId);
 ...
 * Access a resource
 */
router.get('/resources/:resourceId/content', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const { resourceId } = req.params;
    const userId = req.user?.id || 'unknown';

    const result = await mcpDiscoveryService.accessResource(resourceId, userId);
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to access resource',
    });
  }
});
 ...
 * Get tool categories
 */
router.get('/tools/categories', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const stats = mcpDiscoveryService.getDiscoveryStats();
 ...
 */
router.get('/resources/categories', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const stats = mcpDiscoveryService.getDiscoveryStats();
 ...
 */
router.get('/tools/trending', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const userId = req.user?.id || 'unknown';
 ...
 */
router.get('/resources/recent', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const userId = req.user?.id || 'unknown';
    const limit = req.query.limit ? parseInt(req.query.limit as string, 10) : 10;
 ...
 */
router.post('/tools/:toolId/validate', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const { toolId } = req.params;
    const { parameters } = req.body;
      ...
      if (tool.parameters.required) {
        for (const requiredParam of tool.parameters.required) {
          if (!(requiredParam in parameters)) {
            ...
            validation.errors.push(`Missing required parameter: ${requiredParam}`);
          }
        }
      }
    }
 ...
 */
router.get('/tools/:toolId/history', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const { toolId } = req.params;
    const tool = mcpDiscoveryService.getTool(toolId);
 ...
 */
router.get('/resources/:resourceId/history', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    const { resourceId } = req.params;
    const resource = mcpDiscoveryService.getResource(resourceId);
...
router.get('/export', authenticate, requirePermission('mcp', 'read'), async (req, res) => {
  try {
    ...
    const resourceResults = await mcpDiscoveryService.searchResources({}, userId);
    const stats = mcpDiscoveryService.getDiscoveryStats();
    ...
    if (format === 'json') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="mcp-discovery-${Date.now()}.json"`);
      res.json(exportData);
    } else {
      res.status(400).json({
        success: false,
