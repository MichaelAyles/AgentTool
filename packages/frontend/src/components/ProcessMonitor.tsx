...
export function ProcessMonitor() {
  ...
  useEffect(() => {
    ...
    const handleMetricsUpdate = (data: { health: HealthStatus; metrics: ProcessMetrics[] }) => {
    ...
    return () => {
      ...
      socket.off('process:resource-limit-exceeded', handleResourceLimitExceeded);
  ...
  const terminateSession = (sessionId: string) => {
    if (socket && window.confirm('Are you sure you want to terminate this session?')) {
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
            ...
            <div className={`text-lg font-semibold ${health.healthy ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
            ...
            <div className={`text-lg font-semibold ${getUsageColor(health.averageMemoryUsage, limits.maxMemoryMB)}`}>
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
            ...
            <div className={`text-lg font-semibold ${getUsageColor(health.averageCpuUsage, limits.maxCpuPercent)}`}>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-6">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-3">Resource Limits</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="text-gray-500 dark:text-gray-400">Max Memory:</span>
            ...
            <div>
              <span className="text-gray-500 dark:text-gray-400">Max Runtime:</span>
              <span className="ml-2 font-medium">{formatRuntime(limits.maxRuntimeMs)}</span>
            </div>
            <div>
              <span className="text-gray-500 dark:text-gray-400">Max Idle:</span>
              <span className="ml-2 font-medium">{formatRuntime(limits.maxIdleTimeMs)}</span>
            </div>
          </div>
        </div>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {metrics.map((metric) => (
                    <tr key={metric.sessionId}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStateColor(metric.state)}`}>
                          {metric.state}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={getUsageColor(metric.memoryUsage, limits.maxMemoryMB)}>
                          {metric.memoryUsage.toFixed(1)} MB
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <span className={getUsageColor(metric.cpuUsage, limits.maxCpuPercent)}>
