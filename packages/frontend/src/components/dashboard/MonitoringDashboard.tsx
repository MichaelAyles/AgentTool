...
const MonitoringDashboard: React.FC<MonitoringDashboardProps> = ({
  ...
  const [selectedMetricView, setSelectedMetricView] = useState<
    'overview' | 'detailed'
  >('overview');
  const [hiddenAlertTypes, setHiddenAlertTypes] = useState<Set<string>>(
    new Set()
  );
  ...
  const filteredAlerts = alerts.filter(
    alert => !hiddenAlertTypes.has(alert.type) && !alert.acknowledged
  );
              ...
              <div className='flex items-center space-x-3'>
                ...
                <div>
                  ...
                  <div className='text-sm text-gray-500 dark:text-gray-400'>
                    {serviceMetrics.backend.requestsPerMinute} req/min •{' '}
                    {serviceMetrics.backend.averageResponseTime}ms avg
                  </div>
                </div>
              </div>
              <div
                className={`text-sm font-medium ${getStatusColor(serviceMetrics.backend.status)}`}
              >
            ...
            <div className='flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg'>
              <div className='flex items-center space-x-3'>
                ...
                <div>
                  ...
                  <div className='text-sm text-gray-500 dark:text-gray-400'>
                    {serviceMetrics.adapters.healthy}/
                    {serviceMetrics.adapters.total} healthy
                    {serviceMetrics.adapters.errored > 0 && (
                      <span className='text-red-500 ml-1'>
                        • {serviceMetrics.adapters.errored} errors
                      </span>
                    )}
                  </div>
                </div>
