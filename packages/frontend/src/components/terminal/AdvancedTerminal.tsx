...
const AdvancedTerminal: React.FC = () => {
  ...
  const initializeTerminalPane = useCallback((pane: TerminalPane, tab: TerminalTab) => {
    ...
    if (container && pane.terminal) {
      ...
      setTimeout(() => {
        setPanes(prev => prev.map(p => 
          p.id === pane.id ? { ...p, connected: true } : p
        ));
        setTabs(prev => prev.map(t => 
          t.id === tab.id ? { ...t, status: 'connected' } : t
        ));
        pane.terminal.writeln('✅ Connected! Ready to code with AI assistance.');
        pane.terminal.write('\r\n$ ');
      }, 1000);
  ...
  const createTerminal = useCallback((tabId: string, projectId: string = 'default', adapter: string = 'claude-code'): TerminalPane => {
  ...
  const createNewTab = useCallback(async (projectId: string = 'default', adapter: string = 'claude-code') => {
    try {
  ...
  const splitPane = useCallback((direction: 'horizontal' | 'vertical') => {
    ...
    // Create new pane
    const newPane = createTerminal(activeTabId, activeTab.projectId, activeTab.adapter);
  ...
  const exportTerminalContent = useCallback((format: 'txt' | 'html') => {
    const activePane = panes.find(pane => pane.tabId === activeTabId);
    if (!activePane) return;

    const content = format === 'html' 
      ? activePane.terminal.getSelectionAsHTML() || 'No selection'
      : activePane.terminal.getSelection() || activePane.terminal.buffer.active.getLine(0)?.translateToString() || '';
    ...
    <div className="flex flex-col h-full bg-gray-900">
            ...
            <div>
              ...
              >
                {[500, 1000, 2000, 5000].map(lines => (
                  <option key={lines} value={lines}>{lines} lines</option>
                ))}
              </select>
            </div>
            <div>
      ...
      <div className="flex items-center justify-between px-4 py-1 bg-gray-800 border-t border-gray-700 text-xs text-gray-400">
        <div className="flex items-center space-x-4">
          ...
          <span>Connected: {activeTabPanes.filter(p => p.connected).length}/{activeTabPanes.length}</span>
          {activeTabId && (
            <span>Active: {tabs.find(t => t.id === activeTabId)?.title || 'None'}</span>
          )}
        </div>
        <div className="flex items-center space-x-2">
