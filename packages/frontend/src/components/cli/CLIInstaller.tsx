...
const CLIInstaller: React.FC<CLIInstallerProps> = ({ supportedCLIs, installedCLIs }) => {
      ...
      api.post(`/api/cli/${cliName}/install`, options),
    {
      onMutate: ({ cliName }) => {
        setInstallationSteps([
          { id: 'check', name: 'Checking system requirements', status: 'running' },
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg border border-gray-200">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Available CLIs</h3>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="text-sm font-medium text-gray-900">{cliInfo.name}</h4>
                          <p className="text-sm text-gray-500">
                            Command: <code className="px-1 bg-gray-100 rounded">{cliInfo.command}</code>
                          </p>
                          <div className="flex items-center gap-2 mt-1">
                            {cliInfo.platformSupport && (
                              <>
                                {cliInfo.platformSupport.linux && (
                                  <span className="text-xs" title="Linux">🐧</span>
                                )}
                                {cliInfo.platformSupport.darwin && (
                                  <span className="text-xs" title="macOS">🍎</span>
                                )}
                                {cliInfo.platformSupport.win32 && (
                                  <span className="text-xs" title="Windows">🪟</span>
                                )}
                              </>
                            )}
          ...
          {selectedCLI ? (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              {installationSteps.length > 0 && (
                <div className="mb-6">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Installation Progress</h4>
              ...
              <div className="space-y-4 mb-6">
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Installation Command</h4>
                  <code className="block p-3 bg-gray-900 text-gray-100 rounded text-sm">
                    {supportedCLIs?.find(c => c.name === selectedCLI)?.info.installCommand}
                  </code>
                </div>

                {/* Dependencies */}
                {supportedCLIs?.find(c => c.name === selectedCLI)?.info.dependencies && (
                  <div>
                    ...
                    <ul className="space-y-1">
                      {supportedCLIs
                        ...
                        ?.info.dependencies.map((dep: string) => (
                          <li key={dep} className="text-sm text-gray-600 flex items-center gap-2">
                  ...
                  {showAdvancedOptions && (
                    <div className="mt-3 p-3 bg-gray-50 rounded-lg space-y-2">
                      <label className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">Use fallback installation method</span>
                      </label>
                      <label className="flex items-center gap-2">
                        <input type="checkbox" className="rounded" />
                        <span className="text-sm text-gray-700">Skip verification</span>
                      </label>
                      <label className="flex items-center gap-2">
                        <input type="checkbox" className="rounded" defaultChecked />
                        <span className="text-sm text-gray-700">Auto-detect platform</span>
                      </label>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-2">
                {getInstallStatus(selectedCLI) === 'installed' ? (
                  ...
                  <>
                    <button
                      onClick={() => installCLIMutation.mutate({ cliName: selectedCLI })}
            ...
            <div className="bg-gray-50 rounded-lg border border-gray-200 p-6 text-center">
              ...
              <p className="text-gray-600">Select a CLI to view installation options</p>
            </div>
          )}
        </div>
      </div>
