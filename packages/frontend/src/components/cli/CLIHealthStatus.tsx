...
const CLIHealthStatus: React.FC<CLIHealthStatusProps> = ({ selectedCLI, onSelectCLI }) => {
  const [monitoringStatus, setMonitoringStatus] = useState<Record<string, boolean>>({});

  // Fetch all health statuses
  const { data: healthStatuses, isLoading, refetch } = useQuery(
    'cli-health-status',
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <h4 className="text-sm font-medium text-gray-700 mb-3">Monitored CLIs</h4>
            <div className="space-y-2">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="text-lg font-semibold capitalize">{detailedHealth.status}</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="text-sm text-gray-600 mb-2">Success Rate</div>
                    <p className="text-lg font-semibold">
                      {(detailedHealth.performance.successRate * 100).toFixed(1)}%
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium">
                        {formatDuration(detailedHealth.performance.averageResponseTime)}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-600">Commands</span>
                      <p className="font-medium">{detailedHealth.performance.commandCount}</p>
                    </div>
                    <div>
                ...
                {/* Health Checks */}
                <div>
                  <h5 className="text-sm font-medium text-gray-700 mb-2">Recent Health Checks</h5>
                  <div className="space-y-2">
                    {detailedHealth.healthChecks.slice(0, 5).map((check, index) => (
                      <div
                        key={index}
                ...
                {/* Errors and Warnings */}
                {(detailedHealth.errors.length > 0 || detailedHealth.warnings.length > 0) && (
                  <div className="space-y-2">
                    {detailedHealth.errors.map((error, index) => (
                      <div key={index} className="p-2 bg-red-50 text-red-700 text-sm rounded">
                        {error}
                      </div>
                    ))}
                    {detailedHealth.warnings.map((warning, index) => (
                      <div key={index} className="p-2 bg-yellow-50 text-yellow-700 text-sm rounded">
                        {warning}
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => healthCheckMutation.mutate(detailedHealth.name)}
