...
const GitBranchViewer: React.FC<GitBranchViewerProps> = ({
  ...
  const localBranches = branches.filter(b => b.type === 'local' || b.type === 'both');
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading && branches.length === 0 ? (
          ...
          <div className="p-8 text-center text-gray-500 dark:text-gray-400">
            {branches.length === 0 ? 'No branches found' : 'No branches match your filters'}
          </div>
        ) : (
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {filteredBranches.map((branch) => (
              <div key={branch.fullName} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                <div
                  className={`flex items-center p-4 cursor-pointer ${
                    selectedBranch === branch.name ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                  }`}
                  onClick={() => {
                    onBranchSelect?.(branch);
                    setExpandedBranch(expandedBranch === branch.name ? null : branch.name);
                  }}
                >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1">
                        <span className={`font-medium truncate ${
                          branch.current ? 'text-green-700 dark:text-green-400' : 'text-gray-900 dark:text-white'
                        }`}>
                          {branch.name}
                        </span>
                        
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getBranchTypeColor(branch.type)}`}>
                          {branch.type}
                        </span>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="truncate max-w-40">{branch.lastCommit.message}</span>
                        </div>
                        ...
                        {(branch.ahead || branch.behind) && (
                          <div className="flex items-center space-x-1">
                            {branch.ahead && <span className="text-green-600 dark:text-green-400">↑{branch.ahead}</span>}
                            {branch.behind && <span className="text-red-600 dark:text-red-400">↓{branch.behind}</span>}
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {showActions && (
                    <div className="flex items-center space-x-1 ml-2">
                      ...
                      {!branch.current && !branch.protected && onBranchDelete && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <div className="text-sm">
                          <span className="font-medium text-gray-900 dark:text-white">Full Name:</span>
                          <span className="ml-2 font-mono text-gray-600 dark:text-gray-400">{branch.fullName}</span>
                        </div>
                        
                        {branch.tracking && (
                          <div className="text-sm">
                            <span className="font-medium text-gray-900 dark:text-white">Tracking:</span>
                            <span className="ml-2 font-mono text-gray-600 dark:text-gray-400">{branch.tracking}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="text-sm">
                          <span className="font-medium text-gray-900 dark:text-white">Last Commit:</span>
                          <span className="ml-2 font-mono text-gray-600 dark:text-gray-400">{branch.lastCommit.shortHash}</span>
                        </div>
                        
                        <div className="text-sm">
                          <span className="font-medium text-gray-900 dark:text-white">Commit Date:</span>
