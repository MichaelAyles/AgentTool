...
const GitHistoryViewer: React.FC<GitHistoryProps> = ({
  ...
  const filteredCommits = commits.filter(commit => {
    ...
    if (authorFilter && !commit.author.name.toLowerCase().includes(authorFilter.toLowerCase())) {
      return false;
    }
          ...
          <div className="flex items-center space-x-3">
            ...
            {branch && (
              <div className="flex items-center space-x-1 px-2 py-1 bg-blue-100 dark:bg-blue-900/20 rounded-full">
                ...
                <span className="text-xs font-medium text-blue-600 dark:text-blue-400">{branch}</span>
              </div>
            )}
          </div>
      ...
      <div className="max-h-96 overflow-y-auto">
        {loading && commits.length === 0 ? (
          <div className="flex items-center justify-center p-8">
            ...
            <span className="ml-2 text-gray-600 dark:text-gray-400">Loading commits...</span>
          </div>
        ) : filteredCommits.length === 0 ? (
          <div className="p-8 text-center text-gray-500 dark:text-gray-400">
            {commits.length === 0 ? 'No commits found' : 'No commits match your filters'}
          </div>
        ) : (
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {filteredCommits.map((commit) => (
              <div key={commit.hash} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                {/* Commit Header */}
                <div
                  className={`flex items-center p-4 cursor-pointer ${
                    selectedCommit === commit.hash ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                  }`}
                  onClick={() => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2 text-sm">
                          ...
                          <span className="font-medium text-gray-900 dark:text-white">Hash:</span>
                          <span className="font-mono text-gray-600 dark:text-gray-400">{commit.hash}</span>
                        </div>
                        
                        {commit.parents.length > 0 && (
                          <div className="flex items-center space-x-2 text-sm">
                            <ArrowRight className="w-4 h-4 text-gray-400" />
                            <span className="font-medium text-gray-900 dark:text-white">Parents:</span>
                            <div className="flex space-x-1">
                              {commit.parents.map(parent => (
                                <span key={parent} className="font-mono text-gray-600 dark:text-gray-400">
                                  {parent.slice(0, 7)}
                                </span>
                              ))}
                            </div>
