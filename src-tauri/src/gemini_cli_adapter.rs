...
impl GeminiCliAdapter {
    ...
    pub async fn execute_task(
        ...
        // Send task to Gemini CLI process
        if let Some(stdin) = process.child.stdin.as_mut() {
            stdin.write_all(gemini_prompt.as_bytes()).await
                .map_err(|e| anyhow::anyhow!("Failed to write to process: {}", e))?;
            stdin.write_all(b"\n").await?;
            stdin.flush().await?;
        }

        // Read response from Gemini CLI
            ...
            for _ in 0..20 {
                ...
                match reader.read_line(&mut line).await {
                    ...
                    Ok(_) => {
                        ...
                        if line.trim().ends_with(">>> ") || line.trim().ends_with("Complete.") {
                            break;
                        }
                    },
                    Err(_) => break,
        ...
        // Clean up the output
        let cleaned_output = self.clean_gemini_output(&output);
        ...
        Ok(TaskResult {
            ...
            task_description: task.to_string(),
            agent_type: "gemini_cli".to_string(),
            status: if cleaned_output.is_empty() { TaskStatus::Failed } else { TaskStatus::Completed },
            result: if cleaned_output.is_empty() { None } else { Some(cleaned_output.clone()) },
            error: if cleaned_output.is_empty() { Some("No output received".to_string()) } else { None },
            created_at: chrono::Utc::now(),
            completed_at: Some(chrono::Utc::now()),
        })
    }
